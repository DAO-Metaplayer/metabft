...
// newMockCluster creates a new METABFT cluster
func newMockCluster(
	numNodes uint64,
	...
	for index := 0; index < int(numNodes); index++ {
		...
		// Create a new instance of the METABFT node
		nodes[index] = NewMETABFT(logger, backend, transport)
...
// mockCluster represents a mock METABFT cluster
type mockCluster struct {
	nodes []*METABFT        // references to the nodes in the cluster
...
func (m *mockCluster) runSequence(height uint64) {
	...
	for nodeIndex, node := range m.nodes {
		...
		go func(ctx context.Context, node *METABFT) {
